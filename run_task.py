...
def _run_model_tasks(parsed_args):
    ...
    print(f"> Loading input data from '{parsed_args.input_file}'...")
    input_data = pd.read_csv(parsed_args.input_file, sep='\t')
    print("> Transforming input data...")
    input_data = _transform_input_data(input_data)
    print("> Inferring...")
    predictions = trained_model.predict(input_data)
    print(f"> Writing inference result to '{parsed_args.output_file}'...")
